...
Simulates real-time data streaming to test the frontend
"""
...
async def simulate_prepared_payloads():
    """Simulate prepared trading payloads ready for execution"""
    while True:
        if random.random() < 0.1:  # 10% chance of payload
            symbol = random.choice(["EURUSD", "GBPUSD", "USDJPY"])
            side = random.choice(["buy", "sell"])
            ...
            await broadcast_prepared_payload(payload_data)
            print(
                f"🚀 Payload: {side.upper()} {symbol} {payload_data['volume']} lots (R:R {payload_data['risk_reward_ratio']})"
            )

        await asyncio.sleep(12)  # Payloads every 12 seconds
