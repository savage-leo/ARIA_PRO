...
from backend.automation.cicd_pipeline import cicd_pipeline
...
async def get_pipeline_status():
    ...
    try:
        status = cicd_pipeline.get_pipeline_status()
...
async def start_pipeline_scheduler():
    ...
    try:
        await cicd_pipeline.start_scheduler()
...
async def stop_pipeline_scheduler():
    ...
    try:
        await cicd_pipeline.stop_scheduler()
...
async def trigger_pipeline_run(force: bool = False):
    ...
    try:
        ...
        asyncio.create_task(cicd_pipeline.run_pipeline(force))
...
async def get_pipeline_runs(limit: int = 10):
    ...
    try:
        runs = cicd_pipeline.pipeline_history[-limit:] if cicd_pipeline.pipeline_history else []
...
async def get_pipeline_run_details(run_id: str):
    ...
    try:
        run = next((r for r in cicd_pipeline.pipeline_history if r.run_id == run_id), None)
...
async def update_pipeline_config(config_data: Dict[str, Any]):
    ...
    try:
        ...
        if "retrain_interval_hours" in config_data:
            cicd_pipeline.config.retrain_interval_hours = config_data["retrain_interval_hours"]
        if "validation_threshold" in config_data:
            cicd_pipeline.config.validation_threshold = config_data["validation_threshold"]
        if "stress_test_threshold" in config_data:
            cicd_pipeline.config.stress_test_threshold = config_data["stress_test_threshold"]
        if "performance_degradation_limit" in config_data:
            cicd_pipeline.config.performance_degradation_limit = config_data["performance_degradation_limit"]
        if "backup_retention_days" in config_data:
            cicd_pipeline.config.backup_retention_days = config_data["backup_retention_days"]
        if "enable_auto_deployment" in config_data:
            cicd_pipeline.config.enable_auto_deployment = config_data["enable_auto_deployment"]
        if "enable_rollback_on_failure" in config_data:
            cicd_pipeline.config.enable_rollback_on_failure = config_data["enable_rollback_on_failure"]
        if "notification_webhook" in config_data:
            cicd_pipeline.config.notification_webhook = config_data["notification_webhook"]
        if "data_prep_timeout" in config_data:
            cicd_pipeline.config.data_prep_timeout = config_data["data_prep_timeout"]
        if "training_timeout" in config_data:
            cicd_pipeline.config.training_timeout = config_data["training_timeout"]
        if "validation_timeout" in config_data:
            cicd_pipeline.config.validation_timeout = config_data["validation_timeout"]
        if "stress_timeout" in config_data:
            cicd_pipeline.config.stress_timeout = config_data["stress_timeout"]
        if "profiling_timeout" in config_data:
            cicd_pipeline.config.profiling_timeout = config_data["profiling_timeout"]
        if "deployment_timeout" in config_data:
            cicd_pipeline.config.deployment_timeout = config_data["deployment_timeout"]
        if "post_validation_timeout" in config_data:
            cicd_pipeline.config.post_validation_timeout = config_data["post_validation_timeout"]
